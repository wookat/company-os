...
- QA157: backend-only rounds can skip UI entirely — `POST /api/register` for a Bearer token, `GET /api/real/kp?name=<考点>` to build a paper, `GET /api/papers/<id>` for questions, `POST /api/papers/<id>/submit` (`{answers:{qid:'A'},duration_sec:n}`) to seed wrong-book entries, then hit the target endpoint and assert on the JSON response. Browser account switching: `localStorage.setItem('zt_token',…)` + reload. CDP gotcha: two concurrent websockets can make Runtime.evaluate silently drop responses — keep the event-listener and evaluation connections separate or serialize them.
