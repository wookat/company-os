...
2. **平台级组件优先于单款特效**（评分报告系统性结论：69 款工厂壳 C 维度全体 2.5）。以下 6 件一次开发、10 款首发挂载、其余 90 款渐进接入：
   ...
   - **SHELL-6 难度日历**：星期分档统一约定——周一最易→周六最难、周日「变体日」（每款定义自己的变体），难度标签外显在标题与分享卡。
3. **进阶模式统一命名**：每款标配「Daily（默认）/ Hard（同种子高约束）/ Endless（无尽种子赛，`seed=date+run#`）」三档，入口在结算页（完赛才解锁当日 Hard——次日钩子 + 会话延长）。
...
- 单款验收线：Complete/Start 提升 ≥50%（craft 目标 2.1%→10%+）；分享卡双格式可用；390px 无横向滚动；Lighthouse perf ≥90；单局仍在 2-5 分钟。
