...
<input id="kpq" type="search" placeholder="🔍 输入关键词筛选考点，如「矛盾」「共同体」" class="mt-4 w-full sm:max-w-sm h-11 px-4 rounded-xl bg-white border border-black/10 shadow-card text-sm outline-none focus:border-rose-300" oninput="(function(v){v=v.trim().toLowerCase();document.querySelectorAll('.kpchip').forEach(function(a){a.style.display=!v||a.textContent.toLowerCase().includes(v)?'':'none'});document.querySelectorAll('h2[id^=s]').forEach(function(h){var d=h.nextElementSibling;var vis=d&&[...d.children].some(function(c){return c.style.display!=='none'});h.style.display=vis?'':'none';if(d)d.style.display=vis?'':'none'})})(this.value)">
...
<p class="text-xs text-slate-500 font-num"><a class="inline-flex items-center min-h-[32px] -my-2 hover:text-rose-600 underline decoration-dotted underline-offset-2" href="/zhenti/${q.year}#q${q.seq}">${q.year} 年第 ${q.seq} 题</a> · ${q.qtype === "multi" ? "多选" : "单选"} · ${hesc(q.subject || "")}</p>
...
<p class="mt-2.5"><a href="/app#realsubj/${year}-${s.seq}" class="inline-flex items-center min-h-[32px] text-xs text-rose-600 underline decoration-dotted underline-offset-2">背这道参考要点（免费）›</a></p>
</article>`;
    }).join("")}</div>`;
  })()}
