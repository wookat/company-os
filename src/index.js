...
<div class="mt-6 grid gap-3 sm:grid-cols-2">${years.map(y => `<div class="bg-white rounded-2xl border border-black/5 shadow-card p-4 flex items-center justify-between gap-3"><div><p class="text-sm font-bold font-num">${y} 年考研政治真题</p><p class="mt-0.5 text-xs text-slate-500">客观题全量 · 含答案解析</p></div><span class="flex shrink-0 items-center gap-2"><a class="inline-flex items-center min-h-[36px] px-3 rounded-full bg-rose-500 hover:bg-rose-600 text-white text-xs font-semibold" href="/pdf/zhenti-${y}.pdf" download>PDF ↓</a><a class="inline-flex items-center min-h-[36px] px-2 text-xs text-slate-500 hover:text-slate-700 underline decoration-dotted underline-offset-2" href="/zhenti/${y}">在线看 ›</a></span></div>`).join("")}</div>
