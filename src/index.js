...
<p class="mt-2 text-sm text-slate-500">2010-2025 历年真题按官方考点整理，点考点看该考点全部真题（含答案与原创解析）。<a class="text-rose-600 underline" href="/zhenti">按年份看 →</a> · <a class="text-rose-600 underline" href="/zhenti/fenxiti">历年分析题及参考答案 →</a></p>
...
<p class="mt-2 text-sm text-slate-500">2010-2025 考研政治${hesc(sub)}客观题 ${total} 道、覆盖 ${kps.results.length} 个考点，含答案与原创解析。<a class="inline-flex items-center min-h-[32px] py-1.5 text-rose-600 underline font-medium" href="/app#realrand">注册后免费在线抽练判分 →</a></p>
...
${(() => {
      const faqs = [
        [`考研政治${sub}客观题一共考了多少道？`, `2010-2025 共 ${total} 道（不含第三方版权材料题），覆盖 ${kps.results.length} 个考点，本页按考点与年份两个维度全部收录。`],
...
<script type="application/ld+json">${JSON.stringify({ "@context": "https://schema.org", "@type": "ItemList", name: `考研政治${sub}考点真题列表`, numberOfItems: kps.results.length, itemListElement: kps.results.map((r, i) => ({ "@type": "ListItem", position: i + 1, name: r.kp_name, url: `https://zhenti.zalize.com/zhenti/kaodian/${encodeURIComponent(r.kp_name)}` })) })}</script>`;
    return zhentiShell(`考研政治${sub}历年真题及答案（2010-2025 共 ${total} 道）· 真题工坊`, `考研政治${sub}客观题 2010-2025 共 ${total} 道全收录，按 ${kps.results.length} 个考点与年份导航，含答案与原创解析，免费在线刷题判分。`, canon, body, zhentiCrumbs([["首页", "https://zhenti.zalize.com/"], ["历年真题库", "https://zhenti.zalize.com/zhenti"], [`${sub}真题`, canon]]));
  }
...
<div class="mt-3"><p class="text-xs font-semibold text-slate-500">跳到题号<span class="ml-1 font-normal text-slate-400">（玫红为分析题）</span></p><div class="mt-1.5 flex flex-wrap gap-1.5">${qs.results.map(q => `<a href="#q${q.seq}" class="inline-flex items-center justify-center min-w-[32px] min-h-[32px] px-1 rounded-lg bg-white border border-black/5 shadow-card text-xs font-num text-slate-600 hover:border-rose-200 hover:text-rose-600">${q.seq}</a>`).join("")}${sj.results.map(s => `<a href="#q${s.seq}" class="inline-flex items-center justify-center min-w-[32px] min-h-[32px] px-1 rounded-lg bg-rose-50 border border-rose-100 text-xs font-num text-rose-600 hover:bg-rose-100" title="分析题">${s.seq}</a>`).join("")}</div></div>
<div class="mt-6 flex items-center gap-2"><button id="stbtn" onclick="stToggle()" class="min-h-[36px] px-4 py-1.5 inline-flex items-center rounded-full border border-rose-200 bg-rose-50/50 hover:bg-rose-50 text-xs font-semibold text-rose-600">✏️ 自测模式：隐藏答案</button><span class="text-xs text-slate-400">隐藏 ✓ 与解析，先自己做一遍</span></div>
<style>body.selftest{padding-bottom:4.5rem}.selftest .ansmark{visibility:hidden}.selftest .ansbox{display:none}.selftest .ansok{color:#475569;font-weight:400}</style>
<button id="stfab" onclick="stToggle()" class="hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-40 min-h-[36px] px-4 py-1.5 items-center rounded-full bg-rose-500 hover:bg-rose-600 text-white text-xs font-semibold shadow-lg">✏️ 自测中 · 点我显示答案</button>
<script>function stApply(on){document.body.classList.toggle('selftest',on);document.getElementById('stbtn').textContent=on?'👁 显示答案（自测中）':'✏️ 自测模式：隐藏答案';var f=document.getElementById('stfab');f.classList.toggle('hidden',!on);f.classList.toggle('inline-flex',on)}
  ...
  async fetch(request, env, ctx) {
    ...
    if (p === "/sitemap.xml") {
      ...
      const lines = [
        ...
        ...Array.from({ length: 16 }, (_, i) => 2025 - i).map(y => u(`${base}/zhenti/fenxiti/${y}`, "0.7", "yearly")),
